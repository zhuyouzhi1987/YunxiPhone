package com.yunxi.phone.marsdaemon.strategy;
...
import com.yunxi.phone.marsdaemon.IDaemonStrategy;
import com.yunxi.phone.marsdaemon.nativ.NativeDaemonAPI20;
...
public class DaemonStrategyUnder21 implements IDaemonStrategy {
	...
	public void onPersistentCreate(final Context context, final DaemonConfigurations configs) {
		...
		Thread t = new Thread(){
			public void run() {
				...
				Log.e("Daemon", "pkt:"+context.getPackageName()+" srv:" +configs.DAEMON_ASSISTANT_CONFIG.SERVICE_NAME);
				Log.e("Daemon", "binary file path="+binaryFile.getAbsolutePath());
				new NativeDaemonAPI20(context).doDaemon(
						context.getPackageName(), 
						configs.DAEMON_ASSISTANT_CONFIG.SERVICE_NAME,
						binaryFile.getAbsolutePath() );   // "/data/local/tmp/daemon"
			};
		};
	...
	public void onDaemonDead() {
		Log.e("Daemon", "call daemon dead");
		mAlarmManager.setRepeating(AlarmManager.ELAPSED_REALTIME, SystemClock.elapsedRealtime(), 1, mPendingIntent);
		android.os.Process.killProcess(android.os.Process.myPid());
	}
	...
	private boolean installBinary(Context context){
		...
		Log.e("Daemon", "BinaryDirName="+ binaryDirName);
	...
	private void copyFile(File file, InputStream is, String mode) throws IOException, InterruptedException {
		...
		Log.e("Daemon", "Copy file and chmod success!");
	}
}
