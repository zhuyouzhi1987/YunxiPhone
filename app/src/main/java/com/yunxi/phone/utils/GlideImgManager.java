...
import com.yunxi.phone.fragment.GlideRoundTransform;

/**
 * Created by bond on 2016/12/28.
 */
...
public class GlideImgManager {
    ...
    public static void glideLoader2(Context context, int url, int erroImg, int emptyImg, ImageView iv, int tag) {
        if (0 == tag) {
            Glide.with(context).load(url).placeholder(emptyImg).error(erroImg).transform(new GlideCircleTransform(context)).into(iv);
        } else if (1 == tag) {
            Glide.with(context).load(url).centerCrop().placeholder(emptyImg).error(erroImg).transform(new GlideRoundTransform(context,10)).into(iv);
        }
    }
}
